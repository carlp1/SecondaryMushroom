...
if __name__ == '__main__':
    ...
    # 5-Fold Cross Validation with DecisionTree (gini criterion & impurity_threshold = 0.25)
    ...
    last_fold_tree = None  # We'll keep the tree from the last fold for visualization
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X_np)):
        ...
        # Initialize and train the DecisionTree with gini impurity and impurity_threshold = 0.25
        tree = DecisionTree(criterion='gini', impurity_threshold=0.25)
        ...
        # Record tree depth and number of leaves for this fold
        tree_depths.append(tree.tree_depth)
        leaf_counts.append(tree.get_leaf_count())
        ...
        # Save the last fold's results for confusion matrix and tree visualization
        if fold == 4:
            last_fold_tree = tree
            last_fold_y_test = y_test
            last_fold_y_pred = y_test_pred
    ...
    print(f"Average Testing Loss:  {avg_test_loss:.5f}")
    print(f"Average Tree Depth:    {avg_tree_depth:.2f}")
    print(f"Average Leaf Count:    {avg_leaf_count:.2f}")

    # Print the confusion matrix for the last fold
    ...
    # Visualize the tree from the last fold
    print("\nVisualizing the Decision Tree from the last fold...")
    dot = last_fold_tree.visualize_tree()
    dot.format = 'png'
